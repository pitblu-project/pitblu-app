...
type ProbePlan = {coreDeviceId: string; probeChannel: number; purpose: 'pit' | 'food'; label?: string; target?: number; minimum?: number; maximum?: number};
  ...
  const load = useCallback(async () => {
    try {
      ...
      if (identity.role === 'follower') {
        ...
        if (identity.role === 'operator') setCookerProfiles(await api.request<CookerProfile[]>('/api/v1/cooker-profiles'));
  ...
  if (!cook) {
    ...
    return <Shell now={now}><StartCook history={history} system={system!} cookers={cookerProfiles} api={api} onRefresh={load} onCreate={async (name, cookerProfileId, anticipatedServeAt, plans) => {
      ...
      for (const plan of plans) {
        ...
        const measurement = await api.request<Measurement>(`/api/v1/cooks/${created.id}/measurements`, {method: 'POST', body: json(plan.purpose === 'pit' ? {label: `${created.cookers[0]?.name ?? 'Pit'} temperature`, kind: 'cooker', cookerId: created.cookers[0]?.id ?? null, rangeMinC: plan.minimum, rangeMaxC: plan.maximum} : {label: plan.label, kind: 'food', foodItemId, targetTemperatureC: plan.target, approachingMarginC: 3})});
...
function StartCook({history: cooks, system, cookers, api, onRefresh, onCreate}: {history: Cook[]; system: SystemState; cookers: CookerProfile[]; api: PitbluApi; onRefresh: () => Promise<void>; onCreate: (name: string, cookerProfileId: string, anticipatedServeAt: string | null, plans: ProbePlan[]) => Promise<void>}) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {event.preventDefault(); const form = new FormData(event.currentTarget); const plans = sources.flatMap(source => {const key = `${source.deviceId}-${source.probe}`; const purpose = form.get(`purpose-${key}`); if (purpose !== 'pit' && purpose !== 'food') return []; return [{coreDeviceId: source.deviceId, probeChannel: source.probe, purpose, label: String(form.get(`label-${key}`) ?? ''), target: Number(form.get(`target-${key}`)), minimum: Number(form.get(`min-${key}`)), maximum: Number(form.get(`max-${key}`))} satisfies ProbePlan];}); const serve = form.get('serve'); await onCreate(String(form.get('name')), String(form.get('cooker') ?? ''), serve ? new Date(String(serve)).toISOString() : null, plans);};
  return <><section className="welcome"><img src="/pitblu-logo.png" alt="Pitblu smoker logo"/><div><div className="eyebrow">Let’s cook</div><h1>Set up the probes</h1><p>Choose what each connected probe is watching, set the temperatures, then light the fire.</p></div></section><HardwareReadiness system={system}/><form className="quick-start" onSubmit={submit}><section className="card cook-basics"><span className="step">Today’s cook</span><div className="form-row"><label>Cook name<input name="name" required maxLength={120} placeholder="Sunday ribs"/></label><label>Barbecue<select name="cooker" required><option value="">Choose a saved barbecue</option>{cookers.map(cooker => <option key={cooker.id} value={cooker.id}>{cooker.name}</option>)}</select></label></div><label>Eating around <small>Optional</small><input name="serve" type="datetime-local"/></label>{!cookers.length && <p className="nudge">Add your WSM, kettle or other barbecue below first—you only need to do this once.</p>}</section><section><span className="step">Connected probes</span><h2>What is each probe watching?</h2>{sources.length ? <div className="probe-grid">{sources.map(source => <ProbePlanCard key={`${source.deviceId}:${source.probe}`} source={source}/>)}</div> : <div className="card"><p>No probes are connected. You can still start and add probes later.</p></div>}</section><button className="fire-button">🔥 Start cooking</button></form><details className="gear-drawer" open={!cookers.length}><summary>Manage my barbecues</summary><CookerLibrary cookers={cookers} api={api} onRefresh={onRefresh}/></details><section className="past-cooks"><h2>Previous cooks</h2><div className="grid">{cooks.filter(cook => cook.state === 'closed').map(cook => <a className="card history" href={`/?cook=${cook.id}`} key={cook.id}><span>🔥</span><h3>{cook.name}</h3><p>Served {time(cook.servedAt)}</p></a>)}</div></section></>;
}

function ProbePlanCard({source}: {source: {deviceId: string; probe: number; temperatureC: number | null}}) {
  const [purpose, setPurpose] = useState('unused');
  const key = `${source.deviceId}-${source.probe}`;
  return <article className={`card probe-plan ${purpose}`}><div className="probe-heading"><span><strong>Probe {source.probe}</strong><small>{source.deviceId}</small></span><b>{source.temperatureC}°C</b></div><label>Use this for<select name={`purpose-${key}`} value={purpose} onChange={event => setPurpose(event.target.value)}><option value="unused">Not used today</option><option value="food">Food</option><option value="pit">Barbecue temperature</option></select></label>{purpose === 'food' && <div className="probe-options"><label>What food?<input name={`label-${key}`} required placeholder="Brisket flat"/></label><label>Ready at °C<input name={`target-${key}`} type="number" step=".1" required placeholder="93"/></label></div>}{purpose === 'pit' && <div className="probe-options"><label>Keep above °C<input name={`min-${key}`} type="number" required defaultValue="105"/></label><label>Keep below °C<input name={`max-${key}`} type="number" required defaultValue="135"/></label></div>}</article>;
}
