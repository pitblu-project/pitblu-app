...
function Shell({now, children, home = false, activeNav}: {now: Date; children: React.ReactNode; home?: boolean; activeNav?: 'home' | 'cook' | 'history' | 'more'}) {
  ...
  return <div className={`app-shell ${home ? 'home-shell' : ''}`}><header><a href="/" className="brand" aria-label="Pitblu home"><img className="brand-lockup" src="/pitblu-header-logo.png" alt="Pitblu"/></a><nav aria-label="Primary"><a className={selected === 'home' ? 'active' : ''} href="/">⌂ <span>Home</span></a><a className={selected === 'cook' ? 'active' : ''} href="/?view=start">♨ <span>Cook</span></a><a className={selected === 'history' ? 'active' : ''} href="/?view=history">◷ <span>History</span></a><a className={selected === 'more' ? 'active' : ''} href="/?view=more">••• <span>More</span></a></nav><div className="header-status"><i className="status-dot live"/> Pitblu connected</div><time>{now.toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})}</time></header><main>{children}</main><footer>Pitblu · Open source{publicView && <> · <a href="https://github.com/pitblu-project/pitblu-app" target="_blank" rel="noreferrer">View Pitblu on GitHub</a></>}</footer></div>;
}
